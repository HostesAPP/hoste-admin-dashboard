...
export default function SettingsPage() {
  ...
  // Business Rules State (PRD §6.13)
  const [commOneTime, setCommOneTime] = useState("20");
  const [commFullTime, setCommFullTime] = useState("35");
  const [commContract, setCommContract] = useState("30");
  const [commGroup, setCommGroup] = useState("15");
  const [leaderBonus, setLeaderBonus] = useState("5");
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        {/* PRD §6.13 Commission Rates by Booking Type */}
        <div className="bg-card border border-border/80 rounded-2xl p-6 space-y-4 shadow-2xs">
          <h2 className="text-sm font-bold text-foreground border-b border-border/60 pb-3 flex items-center gap-2">
            ...
            <span>Commission Rates by Booking Type (PRD §6.13)</span>
          </h2>

          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
            <div className="space-y-1">
              <label className="font-semibold text-foreground">One-Time / Part-Time (%)</label>
              <Input
                type="number"
                value={commOneTime}
                onChange={(e) => setCommOneTime(e.target.value)}
                className="h-9 text-xs rounded-xl"
              />
            </div>

            <div className="space-y-1">
              <label className="font-semibold text-foreground">Full-Time Engagement (%)</label>
              <Input
                type="number"
                value={commFullTime}
                onChange={(e) => setCommFullTime(e.target.value)}
                className="h-9 text-xs rounded-xl"
              />
            </div>

            <div className="space-y-1">
              <label className="font-semibold text-foreground">Contract Booking (%)</label>
              <Input
                type="number"
                value={commContract}
                onChange={(e) => setCommContract(e.target.value)}
                className="h-9 text-xs rounded-xl"
              />
            </div>

            <div className="space-y-1">
              <label className="font-semibold text-foreground">Group Base / Leader Bonus (%)</label>
              <div className="flex gap-2">
                ...
                <Input
                  type="number"
                  value={leaderBonus}
                  onChange={(e) => setLeaderBonus(e.target.value)}
                  className="h-9 text-xs rounded-xl"
                  placeholder="+5"
                />
              </div>
