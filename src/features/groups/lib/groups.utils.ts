...
// get group leader member
export const getGroupLeaderMember = (groupId: string) => {
  return GROUP_MEMBERS?.find((member) => member.groupId === groupId && (member.role === "Group Leader" || member.isLeader));
};
