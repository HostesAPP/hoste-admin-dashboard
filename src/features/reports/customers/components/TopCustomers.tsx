...
interface TopCustomersProps {
  ...
  customerDirectory?: CustomerDirectoryItem[];
...
export const TopCustomers: React.FC<TopCustomersProps> = ({
  ...
  const handleProfileClick = (id: string, name: string) => {
    if (onSelectCustomer && customerDirectory) {
      const found = customerDirectory.find((c) => c.name === name || c.id === id);
      if (found) {
    ...
    <Card className="rounded-2xl border border-border shadow-xs bg-card">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs text-muted-foreground">
          Highest-value and most active accounts across Hosté
        </CardDescription>
      </CardHeader>
          ...
          {customers.map((customer) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                {/* Avatar */}
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${customer.avatarBg}`}
                >
                  {customer.initial}
                </div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground truncate">
                    {customer.location} • {customer.customerType}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-xs pt-0.5">
                ...
                <button
                  type="button"
                  onClick={() => handleProfileClick(customer.id, customer.name)}
