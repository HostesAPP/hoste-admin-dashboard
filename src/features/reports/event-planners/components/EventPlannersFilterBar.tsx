...
import type { EventPlannerReportFilterParams } from "../types/event-planners.types";

interface EventPlannersFilterBarProps {
  filters: EventPlannerReportFilterParams;
  onFilterChange: (filters: Partial<EventPlannerReportFilterParams>) => void;
  onResetFilters: () => void;
}

export const EventPlannersFilterBar: React.FC<EventPlannersFilterBarProps> = ({
  filters,
  onFilterChange,
  onResetFilters,
}) => {
  return (
    <div className="flex flex-wrap items-center gap-3 p-3 rounded-2xl bg-card border border-border shadow-xs">
      ...
      <div className="w-36">
        ...
        >
          ...
          <SelectContent className="rounded-xl">
            ...
            <SelectItem value="Luxury">Luxury & Galas</SelectItem>
      ...
      <div className="w-44">
        <Select
          value={filters.spend}
          onValueChange={(val) => onFilterChange({ spend: val || "₦0 - ₦500M+" })}
        >
          <SelectTrigger className="h-9 text-xs rounded-xl border-border bg-background">
            <SelectValue placeholder="Spend: ₦0 - ₦500M+" />
          </SelectTrigger>
          <SelectContent className="rounded-xl">
            <SelectItem value="₦0 - ₦500M+">Spend: ₦0 - ₦500M+</SelectItem>
      ...
      <div className="relative flex-1 min-w-[200px]">
        ...
        <Input
          placeholder="Search Planner / Agency..."
